...
namespace utilitaire {
	...
	////////////////////////////////////////////////////////////////////////
	///
	/// @fn bool VisiteurSurTable::segmentsIntersect(glm::vec3 s11, glm::vec3 s12, glm::vec3 s21, glm::vec3 s22)
	///
	/// Cette fonction trouve si deux segments se croisent
	///
	/// @param[in] s11 : Premier point du premier segment
	/// @param[in] s12 : Deuxième point du premier segment
	/// @param[in] s21 : Premier point du deuxième segment
	/// @param[in] s22 : Deuxième point du deuxième segment
	///
	/// @return Vrai si segments se croisent, faux autrement.
	///
	////////////////////////////////////////////////////////////////////////
	bool segmentsIntersect(glm::vec3 s11, glm::vec3 s12, glm::vec3 s21, glm::vec3 s22)
	{
		glm::vec3 u1 = s11;
		glm::vec3 v1 = s12 - s11;
		glm::vec3 u2 = s21;
		...
		float d = (-v2.x * v1.z + v1.x * v2.z);
		if (d == 0)
		{
			return false;
		}

		float s = (-v1.z * (u1.x - u2.x) + v1.x * (u1.z - u2.z)) / d;
		float t = (v2.x * (u1.z - u2.z) - v2.z * (u1.x - u2.x)) / d;
	...
	////////////////////////////////////////////////////////////////////////
	///
	/// @fn bool VisiteurSurTable::segmentCercleIntersect(glm::vec3 s1, glm::vec3 s2, glm::vec3 centreCercle, float rayon)
	///
	/// Cette fonction trouve si un segment et un cercle se croisent
	///
	/// @param[in] s1 : Premier point du segment
	/// @param[in] s2 : Deuxième point du segment
	/// @param[in] centreCercle : Centre du cercle
	/// @param[in] rayon : Rayon du cercle
	///
	/// @return Vrai si le segment et le cercle se croisent, faux autrement.
	///
	////////////////////////////////////////////////////////////////////////
	bool segmentCercleIntersect(glm::vec3 s1, glm::vec3 s2, glm::vec3 centreCercle, float rayon)
	{

		glm::vec3 v1 = s2 - s1;
		glm::vec3 v2 = s1 - centreCercle;

		float a = glm::dot(v1, v1);
		float b = 2 * glm::dot(v2, v1);
		float c = glm::dot(v2, v2) - rayon * rayon;

		float disc = std::sqrt((b*b) - (4 * a*c));
	...
	float distancePointSegment(glm::vec3 p1, glm::vec3 p2, glm::vec3 p)
	{
		return glm::length(vectorPointSegment(p1, p2, p));
	}

	glm::vec3 vectorPointSegment(glm::vec3 p1, glm::vec3 p2, glm::vec3 p)
	{
		return p - (p1 + glm::proj(p - p1, p2 - p1));
	}
